...
part 'athelete_event.dart';
part 'athelete_state.dart';
...
class AthletePageBloc extends Bloc<AthletePageEvent, AthletePageState> {
  ...
  AthletePageBloc({required this.repo}) : super(AthletePageState.initial()) {
    on<OnRefresh>(_mapPageRefreshEventToState);
    on<OnGraphRefresh>(_mapGraphRefreshEventToState);
  }

  void _mapPageRefreshEventToState(
      OnRefresh event, Emitter<AthletePageState> emit) async {
    try {
      ...
      final MLBAthleteStats stats =
          await repo.getPlayerStatsHistory(playerId, from, until);
      print("fetched MLBAthlete data");
      final List<GraphData> graphStats = stats.statHistory
          .map((stat) => GraphData(
              DateFormat("yyy-MM-dd").parse(stat.timeStamp), stat.price * 1000))
          .toList();
      Set<DateTime> seenDates = {};
      final List<GraphData> distinctPoints =
