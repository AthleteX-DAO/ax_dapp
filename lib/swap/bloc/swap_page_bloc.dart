...
import 'package:ax_dapp/swap/models/models.dart';
...
part 'swap_page_event.dart';
part 'swap_page_state.dart';

class SwapPageBloc extends Bloc<SwapPageEvent, SwapPageState> {
  SwapPageBloc({
  ...
  FutureOr<void> _onFetchSwapInfoRequested(
    ...
    try {
      ...
      final tokenFromDecimal =
          await _walletRepository.getDecimals(state.tokenFrom.address);
      final tokenToDecimal =
          await _walletRepository.getDecimals(state.tokenTo.address);
      swapRepository
      ...
      if (isSuccess) {
        ...
        // ignore: avoid_print
        print(
            '[SwapPageBloc] fetchSwapInfo failed: ${response.getRight().toNullable()?.errorMsg}',);
        final errorMsg = response.getRight().toNullable()?.errorMsg ?? '';
        final Failure failure;
        if (errorMsg.contains('No pair info') ||
            errorMsg.contains('No swap info')) {
          failure = NoLiquidityPoolFailure();
        } else if (errorMsg.contains('GraphQL') ||
            errorMsg.contains('occurred')) {
          failure = GraphQLFailure(message: errorMsg);
        } else {
      ...
      // ignore: avoid_print
      print('[SwapPageBloc] Exception in fetchSwapInfo: $error');
      emit(
        state.copyWith(
          status: BlocStatus.error,
          failure: UnknownTradeFailure(),
