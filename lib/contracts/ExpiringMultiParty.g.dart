...
class ExpiringMultiParty extends _i1.GeneratedContract {
  ExpiringMultiParty(
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> createLiquidation(
      _i1.EthereumAddress sponsor,
      dynamic minCollateralPerToken,
      dynamic maxCollateralPerToken,
      dynamic maxTokensToLiquidate,
      BigInt deadline,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> transferPositionPassedRequest(
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> withdrawLiquidation(
  ...
  Stream<DisputeSettled> disputeSettledEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('DisputeSettled');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<EmergencyShutdown> emergencyShutdownEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('EmergencyShutdown');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<EndedSponsorPosition> endedSponsorPositionEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('EndedSponsorPosition');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<FinalFeesPaid> finalFeesPaidEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('FinalFeesPaid');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<LiquidationCreated> liquidationCreatedEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('LiquidationCreated');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<LiquidationDisputed> liquidationDisputedEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('LiquidationDisputed');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<LiquidationWithdrawn> liquidationWithdrawnEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('LiquidationWithdrawn');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<NewSponsor> newSponsorEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('NewSponsor');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<PositionCreated> positionCreatedEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('PositionCreated');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<RegularFeesPaid> regularFeesPaidEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('RegularFeesPaid');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<RequestTransferPosition> requestTransferPositionEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('RequestTransferPosition');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<RequestTransferPositionCanceled> requestTransferPositionCanceledEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('RequestTransferPositionCanceled');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<RequestTransferPositionExecuted> requestTransferPositionExecutedEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('RequestTransferPositionExecuted');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<RequestWithdrawal> requestWithdrawalEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('RequestWithdrawal');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<RequestWithdrawalCanceled> requestWithdrawalCanceledEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('RequestWithdrawalCanceled');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<RequestWithdrawalExecuted> requestWithdrawalExecutedEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('RequestWithdrawalExecuted');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
  ...
  Stream<SettleExpiredPosition> settleExpiredPositionEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('SettleExpiredPosition');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
...
class GetOutstandingRegularFees {
  GetOutstandingRegularFees(List<dynamic> response)
      : regularFee = (response[0] as dynamic),
        latePenalty = (response[1] as dynamic),
        totalPaid = (response[2] as dynamic);
...
class Liquidations {
  Liquidations(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        liquidator = (response[1] as _i1.EthereumAddress),
        state = (response[2] as BigInt),
        liquidationTime = (response[3] as BigInt),
        tokensOutstanding = (response[4] as dynamic),
        lockedCollateral = (response[5] as dynamic),
        liquidatedCollateral = (response[6] as dynamic),
        rawUnitCollateral = (response[7] as dynamic),
        disputer = (response[8] as _i1.EthereumAddress),
        settlementPrice = (response[9] as dynamic),
        finalFee = (response[10] as dynamic);
...
class Positions {
  Positions(List<dynamic> response)
      : tokensOutstanding = (response[0] as dynamic),
        withdrawalRequestPassTimestamp = (response[1] as BigInt),
        withdrawalRequestAmount = (response[2] as dynamic),
        rawCollateral = (response[3] as dynamic),
        transferPositionRequestPassTimestamp = (response[4] as BigInt);
...
class Deposit {
  Deposit(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        collateralAmount = (response[1] as BigInt);
...
class DisputeSettled {
  DisputeSettled(List<dynamic> response)
      : caller = (response[0] as _i1.EthereumAddress),
        sponsor = (response[1] as _i1.EthereumAddress),
        liquidator = (response[2] as _i1.EthereumAddress),
        disputer = (response[3] as _i1.EthereumAddress),
        liquidationId = (response[4] as BigInt),
        disputeSucceeded = (response[5] as bool);
...
class EmergencyShutdown {
  EmergencyShutdown(List<dynamic> response)
      : caller = (response[0] as _i1.EthereumAddress),
        originalExpirationTimestamp = (response[1] as BigInt),
        shutdownTimestamp = (response[2] as BigInt);
...
class EndedSponsorPosition {
  EndedSponsorPosition(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress);
...
class FinalFeesPaid {
  FinalFeesPaid(List<dynamic> response) : amount = (response[0] as BigInt);

  final BigInt amount;
}

class LiquidationCreated {
  LiquidationCreated(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        liquidator = (response[1] as _i1.EthereumAddress),
        liquidationId = (response[2] as BigInt),
        tokensOutstanding = (response[3] as BigInt),
        lockedCollateral = (response[4] as BigInt),
        liquidatedCollateral = (response[5] as BigInt),
        liquidationTime = (response[6] as BigInt);
...
class LiquidationDisputed {
  LiquidationDisputed(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        liquidator = (response[1] as _i1.EthereumAddress),
        disputer = (response[2] as _i1.EthereumAddress),
        liquidationId = (response[3] as BigInt),
        disputeBondAmount = (response[4] as BigInt);
...
class LiquidationWithdrawn {
  LiquidationWithdrawn(List<dynamic> response)
      : caller = (response[0] as _i1.EthereumAddress),
        paidToLiquidator = (response[1] as BigInt),
        paidToDisputer = (response[2] as BigInt),
        paidToSponsor = (response[3] as BigInt),
        liquidationStatus = (response[4] as BigInt),
        settlementPrice = (response[5] as BigInt);
...
class NewSponsor {
  NewSponsor(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress);
...
class PositionCreated {
  PositionCreated(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        collateralAmount = (response[1] as BigInt),
        tokenAmount = (response[2] as BigInt);
...
class RegularFeesPaid {
  RegularFeesPaid(List<dynamic> response)
      : regularFee = (response[0] as BigInt),
        lateFee = (response[1] as BigInt);
...
class Repay {
  Repay(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        numTokensRepaid = (response[1] as BigInt),
        newTokenCount = (response[2] as BigInt);
...
class RequestTransferPosition {
  RequestTransferPosition(List<dynamic> response)
      : oldSponsor = (response[0] as _i1.EthereumAddress);

  final _i1.EthereumAddress oldSponsor;
}

class RequestTransferPositionCanceled {
  RequestTransferPositionCanceled(List<dynamic> response)
      : oldSponsor = (response[0] as _i1.EthereumAddress);

  final _i1.EthereumAddress oldSponsor;
}

class RequestTransferPositionExecuted {
  RequestTransferPositionExecuted(List<dynamic> response)
      : oldSponsor = (response[0] as _i1.EthereumAddress),
        newSponsor = (response[1] as _i1.EthereumAddress);
...
class RequestWithdrawal {
  RequestWithdrawal(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        collateralAmount = (response[1] as BigInt);
...
class RequestWithdrawalCanceled {
  RequestWithdrawalCanceled(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        collateralAmount = (response[1] as BigInt);
...
class RequestWithdrawalExecuted {
  RequestWithdrawalExecuted(List<dynamic> response)
      : sponsor = (response[0] as _i1.EthereumAddress),
        collateralAmount = (response[1] as BigInt);
...
class SettleExpiredPosition {
  SettleExpiredPosition(List<dynamic> response)
      : caller = (response[0] as _i1.EthereumAddress),
        collateralReturned = (response[1] as BigInt),
        tokensBurned = (response[2] as BigInt);
